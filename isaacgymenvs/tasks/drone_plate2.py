...
def compute_drone_reward(root_positions, target_root_positions, root_quats, root_linvels, root_angvels, thrusts, reset_buf, progress_buf, max_episode_length):
    ...
    static_reward = torch.where((torch.norm(root_linvels, dim=1) < 0.1) & (torch.norm(root_angvels, dim=1) < 0.1), 1.0 - thrust_penalty, 0.0)

    reward = pos_reward + pos_reward * (up_reward + spinnage_reward) + static_reward
...
'''
python train.py task=Drone checkpoint=runs/Drone_02-18-47-15/nn/Drone.pth test=True num_envs=64
