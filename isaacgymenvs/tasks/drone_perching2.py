...
class Drone(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # Observations:
        # 0:13 - root state
        self.cfg["env"]["numObservations"] = 14  # Updated to include new state

        # Actions:
        self.cfg["env"]["numActions"] = 5  # Updated to include new action
        ...
        self.root_positions_offset = torch.tensor([0.0, 0.0, 1.0], device=self.device)
        
        self.marker_states = vec_root_tensor[:, 1, :]
        self.marker_positions = self.marker_states[:, 0:3]

        # Initialize new state for perching support
        self.perching_support = torch.zeros((self.num_envs, 1), device=self.device)
        ...
        self.perching_support_limit = 50  # Max support force

        # control tensors
        self.thrusts = torch.zeros((self.num_envs, 4), dtype=torch.float32, device=self.device, requires_grad=False)
        self.perching_actions = torch.zeros((self.num_envs, 1), dtype=torch.float32, device=self.device, requires_grad=False)
    ...
    def set_targets(self, env_ids):
        ...
        self.marker_positions[env_ids, 2] += 0.0  # Adjust for better visual alignment
    ...
    def reset_idx(self, env_ids):
        ...
        # Reset the perching support state
        self.perching_support[env_ids] = 0.0
    ...
    def pre_physics_step(self, _actions):
        ...
        # New action for perching support
        perching_action = torch.clamp(actions[:, 4], 0, 1) * self.perching_support_limit

        # Correct scaling of thrust forces
        thrust_prop_0 *= self.dt
        thrust_prop_1 *= self.dt
        thrust_prop_2 *= self.dt
        thrust_prop_3 *= self.dt
        perching_action *= self.dt

        self.forces[:, 1, 2] = thrust_prop_0
        self.forces[:, 2, 2] = thrust_prop_1
        self.forces[:, 3, 2] = thrust_prop_2
        self.forces[:, 4, 2] = thrust_prop_3

        # Apply the perching support force
        self.perching_support[:, 0] = perching_action
        self.forces[:, 0, 2] += perching_action  # Apply the support force to the drone
    ...
    def compute_observations(self):
        ...
        # New state for perching support
        self.obs_buf[..., 13] = self.perching_support[:, 0]
...
@torch.jit.script
def compute_drone_reward(root_positions, target_root_positions, root_quats, root_linvels, root_angvels, perching_support, reset_buf, progress_buf, max_episode_length):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float) -> Tuple[Tensor, Tensor]

    # distance to target
    device = root_positions.device
    root_position_offset = torch.tensor([0.0, 0.0, -1.0], device=device)
    adjusted_positions = root_positions + root_position_offset

    # Distance to target
    target_dist = torch.sqrt(torch.square(target_root_positions - adjusted_positions).sum(-1))
    ...
    spinnage_penalty = 1.0 / (1.0 + spinnage * spinnage)

    # Combined reward
    reward = pos_reward + pos_reward * (up_reward - spinnage_penalty)

    # Additional reward for using perching support effectively
    perching_reward = perching_support[:, 0] * 0.1
    reward += perching_reward

    ones = torch.ones_like(reset_buf)
    die = torch.zeros_like(reset_buf)
    die = torch.where(target_dist > 8.0, ones, die)
    die = torch.where(adjusted_positions[..., 2] < 0.5, ones, die)
...
"""
python train.py task=Drone checkpoint=runs/Drone_02-02-29-54/nn/Drone.pth  test=True num_envs=64
"""