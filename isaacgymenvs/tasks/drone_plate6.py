...
class Drone(VecTask):
    ...
    def set_targets(self, env_ids):
        ...
        self.target_root_positions[env_ids, 2] = torch.zeros(num_sets, device=self.device) + 0
...
def compute_drone_reward(root_positions, target_root_positions, root_quats, root_linvels, root_angvels, thrusts, reset_buf, progress_buf, max_episode_length):
    ...
    pos_reward = 5.0 / (1.0 + target_dist * target_dist)
    ...
    static_reward = torch.where((torch.norm(root_linvels, p=2, dim=1) < 0.1) & (torch.norm(root_angvels, p=2, dim=1) < 0.1), 0.5 - thrust_penalty, 0.0)
