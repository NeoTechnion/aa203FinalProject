...
class Drone(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # Observations:
        self.cfg["env"]["numObservations"] = 17  # Drone states + plate orientation
        ...
        bodies_per_env = 7  # Including the light indicator
        ...
        self.marker_quats = self.marker_states[:, 3:7]
    ...
    def _create_light_indicator(self):
        light_asset_options = gymapi.AssetOptions()
        light_asset_options.fix_base_link = True
        light_asset = self.gym.create_sphere(self.sim, 0.05, light_asset_options)

        default_pose = gymapi.Transform()
        default_pose.p = gymapi.Vec3(0, 0, 2)

        self.light_handles = []
        for i in range(self.num_envs):
            light_handle = self.gym.create_actor(self.envs[i], light_asset, default_pose, "light_indicator", i, 0)
            self.gym.set_rigid_body_color(self.envs[i], light_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(0, 1, 0))
            self.light_handles.append(light_handle)

    def set_light_color(self, color):
        for i in range(self.num_envs):
            self.gym.set_rigid_body_color(self.envs[i], self.light_handles[i], 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
    ...
    def reset_idx(self, env_ids, reset_plates=False):
        ...
        # Reset the plate states at the beginning of an epoch
        if reset_plates:
            for i in range(self.num_envs):
                tilt_angle = np.random.uniform(-0.1, 0.1)  # Small tilt angle in radians
                tilt_direction = np.random.uniform(0, 2 * np.pi)  # Random direction in radians
                quat = gymapi.Quat.from_axis_angle(gymapi.Vec3(np.cos(tilt_direction), np.sin(tilt_direction), 0), tilt_angle)
                self.marker_states[i, 3:7] = torch.tensor([quat.w, quat.x, quat.y, quat.z], dtype=torch.float32, device=self.device)
    ...
    def pre_physics_step(self, _actions):
        ...
        # Apply forces to the appropriate bodies
        self.forces.fill_(0)
        ...
        # Reshape the forces tensor to match the expected shape
        forces_flat = self.forces.view(-1, 3)

        self.gym.apply_rigid_body_force_tensors(self.sim, gymtorch.unwrap_tensor(forces_flat), None, gymapi.LOCAL_SPACE)

    def post_physics_step(self):
        self.progress_buf += 1

        # Check for collisions and update light color
        self.check_collisions_and_update_light()
    ...
    def check_collisions_and_update_light(self):
        ...
        collision_detected = torch.any(torch.norm(contact_forces, dim=2) > 0.0, dim=1)

        if collision_detected.any():
            self.set_light_color(gymapi.Vec3(1.0, 0.0, 0.0))  # Red light for collision
        else:
            self.set_light_color(gymapi.Vec3(0.0, 1.0, 0.0))  # Green light for no collision
    ...
    def compute_observations(self):
        ...
        self.obs_buf[..., 13:17] = self.marker_quats  # Add plate orientation to observations
        return self.obs_buf
    ...
    def compute_reward(self):
        self.rew_buf[:], self.reset_buf[:] = compute_drone_reward(
            ...
            self.root_linvels,
            self.root_angvels,
            self.thrusts,  # Include thrusts in the reward computation
...
@torch.jit.script
def compute_drone_reward(root_positions, target_root_positions, root_quats, root_linvels, root_angvels, thrusts, reset_buf, progress_buf, max_episode_length):
    ...
    # Reward for minimal thrust when static
    thrust_penalty = torch.sum(thrusts, dim=1) / (4 * 2000.0)  # Normalize by max thrust
    static_reward = torch.where((torch.norm(root_linvels, p=2, dim=1) < 0.1) & (torch.norm(root_angvels, p=2, dim=1) < 0.1), 2.0 - thrust_penalty, 0.0)

    # Reward for being near the center of the plate
    center_dist = torch.sqrt(torch.sum((root_positions - target_root_positions) ** 2, dim=1))
    center_reward = torch.where(center_dist < 0.1, 1.0, 0.0)

    # reward = pos_reward + pos_reward * (up_reward + spinnage_reward) + static_reward + center_reward
    reward = pos_reward + pos_reward * (spinnage_reward) + static_reward + center_reward
...
'''
python train.py task=Drone checkpoint=runs/Drone_02-21-30-39/nn/Drone.pth test=True num_envs=64
'''
